 ...
 */
abstract class IntegrationTestCase extends \OTGS\Toolset\PHPUnit\Framework\TestCase
{
    /**
     * @return string
     */
    ...
    protected function doIntegrationTest($file, $message, $condition, $templates, $exception, $outputs)
    {
        ...
        $loader = new \OTGS\Toolset\Twig\Loader\ArrayLoader($templates);
        foreach ($outputs as $i => $match) {
            $config = \array_merge(['cache' => \false, 'strict_variables' => \true], $match[2] ? eval($match[2] . ';') : []);
            $twig = new \OTGS\Toolset\Twig\Environment($loader, $config);
            ...
            $p->setAccessible(\true);
            $p->setValue($twig, '\\OTGS\\Toolset\\__TwigTemplate_' . \hash('sha256', \uniqid(\mt_rand(), \true), \false) . '_');
            try {
                ...
                throw new \OTGS\Toolset\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
            }
            try {
                ...
                $e = new \OTGS\Toolset\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
                $output = \trim(\sprintf('%s: %s', \get_class($e), $e->getMessage()));
            }
            if (\false !== $exception) {
                list($class) = \explode(':', $exception);
                $constraintClass = \class_exists('OTGS\\Toolset\\PHPUnit\\Framework\\Constraint\\Exception') ? 'PHPUnit\\Framework\\Constraint\\Exception' : 'PHPUnit_Framework_Constraint_Exception';
...
\class_alias('OTGS\\Toolset\\Twig\\Test\\IntegrationTestCase', 'OTGS\\Toolset\\Twig_Test_IntegrationTestCase');
